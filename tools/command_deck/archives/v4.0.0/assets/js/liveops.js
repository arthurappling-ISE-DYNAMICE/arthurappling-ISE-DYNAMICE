...
(function () {
    ...
    SYSOS.liveOpsInit = function () {
        SYSOS.router.register({
            id: 'liveops',
            label: '07 // LIVE OPS',
            // v3.3: lazy — shell + listeners + first render on activation.
            render: function (panel) {
                livePanel = panel;
                panel.innerHTML = sectionShell();   // static trusted shell
        ...
        // Refresh only once mounted (render fns guard on missing elements anyway).
        ['sysos:telemetry', 'sysos:ops', 'sysos:compliance', 'sysos:ocr', 'sysos:data']
            .forEach(function (evt) { document.addEventListener(evt, function () { if (livePanel) refreshAll(); }); });
    };
})();
