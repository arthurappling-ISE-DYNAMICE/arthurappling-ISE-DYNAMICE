...
(function () {
    ...
    const commercial = {
        ...
        // v3.3: cached client->proposals/contracts index. Built once, reused across
        // healthDistribution/forecast/workspace; invalidated on any data mutation.
        _clientIndex: null,
        _buildClientIndex() {
            const p = {}, k = {};
            if (proposals()) proposals().list({ includeArchived: true }).forEach(function (x) { (p[x.clientId] = p[x.clientId] || []).push(x); });
            if (contracts()) contracts().list({ includeArchived: true }).forEach(function (x) { (k[x.clientId] = k[x.clientId] || []).push(x); });
            this._clientIndex = { proposals: p, contracts: k };
            return this._clientIndex;
        },
        invalidateClientIndex() { this._clientIndex = null; },

        proposalsFor(clientId) {
            return (this._clientIndex || this._buildClientIndex()).proposals[clientId] || [];
        },
        contractsFor(clientId) {
            return (this._clientIndex || this._buildClientIndex()).contracts[clientId] || [];
        },
        ...
        /** v3.4: memoized health. Returns cached result while the client's data
         *  is unchanged; recomputes only on a per-client token change. Equal to
         *  _computeHealth(id) by construction. */
        health(clientId) {
            const cache = SYSOS.healthCache;
            if (!cache) return commercial._computeHealth(clientId);
            const hit = cache.getHealth(clientId);
            if (hit) return hit;
            const result = commercial._computeHealth(clientId);
            if (result) cache.setHealth(clientId, result);
            return result;
        },

        /** Slow-path: the raw per-client health computation (no cache). */
        _computeHealth(clientId) {
            const c = clients().get(clientId);
            if (!c) return null;
            const signals = [];
            const MAX = 20;
        ...
        healthDistribution() {
            const cache = SYSOS.healthCache;
            if (cache) { const cached = cache.getDist(); if (cached) return cached; }
            const out = { GREEN: 0, YELLOW: 0, RED: 0 };
            clients().list({ includeArchived: true }).forEach(function (c) {
                const h = commercial.health(c.id);   // per-client cache hit when unchanged
                if (h) out[h.status]++;
            });
            if (cache) cache.setDist(out);
            return out;
        },
    ...
    // v3.3: invalidate the client index when proposals/contracts/clients change.
    document.addEventListener('sysos:data', function (e) {
        if (!e.detail || ['proposals', 'contracts', 'clients', 'batch'].indexOf(e.detail.domain) !== -1) {
            commercial.invalidateClientIndex();
        }
    });
})();
