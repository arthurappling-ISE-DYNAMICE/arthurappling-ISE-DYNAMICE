...
(function () {
    ...
    const MATRIX = Object.freeze({
        ...
        'vault.reset':         ['ADMINISTRATOR'],                            // v3.5 H3 — destructive vault wipe (admin-only)
        'vault.attachEvidence':['ADMINISTRATOR', 'MANAGER', 'OPERATOR'],     // v3.5 H5 — seal evidence (aligned with vault.ingest)
        'vault.ocrProvider':   ['ADMINISTRATOR', 'MANAGER'],                 // v3.5 H5 — bind OCR processing capability (config/trust change)
        'backup.export':       ['ADMINISTRATOR', 'MANAGER', 'OPERATOR', 'READ_ONLY'],  // v3.8 — read full state (READ_ONLY may export)
        'backup.restore':      ['ADMINISTRATOR', 'MANAGER', 'OPERATOR']      // v3.8 — destructive restore (READ_ONLY denied; OPERATOR needs confirm)
    });
