    ...
    const maintenance = {
        async run() {
            ...
            const checks = [
                ...
                { name: 'Persistence', ok: S.storage.keys().some(function (k) { return k.indexOf('sysos.reg') === 0; }) || !!S.storage.get(S.CONFIG.STORE.STORAGE_KEY),
                    detail: S.storage.keys().length + ' stores (' + S.storage.backendName + ')' },
