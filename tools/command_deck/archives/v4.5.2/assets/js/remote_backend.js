        ...
        syncToLocal: async function (opts) {
            ...
            try {
                ...
                Object.keys(incoming).forEach(function (k) {
                    if (incoming[k] == null || k.indexOf('sysos.') !== 0) return;
                    let val = incoming[k];
                    // v4.5.2: pulled RLS status can never carry technical proof (same
                    // sanitizer as backup restore — downgrade is audit-logged).
                    if (k === 'sysos.deploy.rls.v1' && SYSOS.environment && SYSOS.environment.sanitizeRLSImport) val = SYSOS.environment.sanitizeRLSImport(val);
                    SYSOS.storage.set(k, val);
                });
                rehydrate();
