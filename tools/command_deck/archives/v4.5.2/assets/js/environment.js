...
(function () {
    ...
    async function runProductionGuard() {
        ...
        const checks = [
            ...
            { name: 'RLS isolation verified',
                ...
                detail: rls.status === RLS_STATUSES.TECHNICALLY_VERIFIED ? ('technically verified ' + (rls.recordedAt || ''))
                    : rls.status === RLS_STATUSES.OPERATOR_ATTESTED ? 'Operator attested; technical RLS verification still required.'
                    : rls.status === RLS_STATUSES.TECHNICAL_VERIFICATION_REQUIRED ? 'imported/downgraded status — local technical re-verification required'
    ...
    const environment = {
        ...
        /**
         * v4.4.3 placeholder for a FUTURE technical verification (e.g. an
         * automated two-session probe proving user A cannot read user B's row).
         * Always returns CONFIG_REQUIRED in this build — no live calls, no
         * credentials, no simulated success. Never sets TECHNICALLY_VERIFIED
         * itself; that would require a real implementation against a live backend.
         */
        verifyRLSIsolationTechnical: function () {
            return { ok: false, status: RLS_STATUSES.CONFIG_REQUIRED,
                detail: 'Automated technical RLS verification is not implemented in this build. Requires a live Supabase project, two authenticated test sessions, and a real cross-account read attempt — none of which exist here.' };
        },
