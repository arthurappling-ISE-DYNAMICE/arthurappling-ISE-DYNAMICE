...
(function () {
    ...
    var MANIFEST = {
        ...
        storage: ['get', 'set', 'remove', 'keys', 'query', 'registerBackend'],
        sqlite: ['migrateAndActivate', 'rollbackToLocal'],
        auth: ['can', 'attribute', 'setOperator', 'permissionsFor'],
        audit: ['record', 'query', 'export', 'restore', 'init'],
        authUI: ['login', 'logout', 'init', 'validate'],
        DataStore: [],
        stores: ['define', 'restore', 'totals'],
        activity: ['log', 'recent', 'restore'],
        relations: ['find', 'resolve', 'backlinks', 'integrity', 'linkStats'],
        canvas: ['init', 'pause', 'resume', 'status'],
        router: ['init', 'switch', 'register'],
        mountTerminals: null,
        vault: ['ingest', 'verifyChain', 'init'],
        registriesData: null,
        registriesInit: null,
        exec: ['init'],
        ...
        commercial: ['setStage', 'forecast', 'health', 'proposalAutomation', 'contractMonitoring', 'portfolio'],
