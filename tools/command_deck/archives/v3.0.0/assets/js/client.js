...
(function () {
    ...
    function renderList() {
        ...
        if (!wrap) return;
        wrap.textContent = '';
        const rows = filteredClients();   // v2.9.8: filter in the data layer, before render
        if (!rows.length) {
    ...
    SYSOS.clientStation = {
        ...
        /** Data-level filter entry point (replaces DOM show/hide). */
        setListFilter(patch) { listFilter = Object.assign({}, listFilter, patch || {}); renderList(); },
