...
(function () {
    ...
    async function runProductionGuard() {
        ...
        const checks = [
            ...
            // v4.6: hosting is BLOCK unless genuinely hosted (GITHUB_PAGES_VERIFIED).
            // Secure localhost used to be WARN — but with RLS now PASS-able, a WARN
            // here could let the guard declare PRODUCTION_GUARD_PASSED on a local
            // tab (Fable review edge). Deploy package is PREPARED; Pages NOT_DEPLOYED.
            { name: 'Hosting / HTTPS', status: hosting.state === 'GITHUB_PAGES_VERIFIED' ? 'PASS' : 'BLOCK',
                detail: hosting.state === 'GITHUB_PAGES_VERIFIED' ? hosting.detail
                    : (hosting.detail + ' — deploy package PREPARED (v4.6); GitHub Pages NOT_DEPLOYED') },
    ...
    const environment = {
        ...
        deploymentReadiness: async function () {
            ...
            const hostingStatus = hosting.state === 'GITHUB_PAGES_VERIFIED' ? 'VERIFIED'
                : hosting.state === 'LOCAL' ? 'LOCAL — deploy PREPARED, Pages NOT_DEPLOYED' : 'CONFIG_REQUIRED';
