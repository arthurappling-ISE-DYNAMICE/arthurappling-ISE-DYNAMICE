...
(function () {
    ...
    const monitoring = {
        ...
        /**
         * v4.3 deployment health probe — reports the hosted-rehearsal state
         * (origin, secure context, profile, backend mode, CSP note, guard status).
         * No external service; captured locally like every other event.
         */
        deploymentHealthCheck: function (extra) {
            // v4.4.3: backend config + CSP-origin state now come from the shared
            // runtime_config authority (was duplicated inline here).
            const bcfg = SYSOS.runtimeConfig.getBackendConfig();
            const csp = SYSOS.runtimeConfig.getCspSupabaseState();
            const hosting = SYSOS.runtimeConfig.getHostingState();
            const info = Object.assign({
                origin: location.origin,
                secureContext: !!window.isSecureContext,
                protocol: location.protocol,
                profile: (SYSOS.environment && SYSOS.environment.getCurrentProfile) ? SYSOS.environment.getCurrentProfile() : 'unknown',
                backendMode: bcfg.MODE || bcfg.BACKEND_MODE || 'local',
                monitoringActive: this.active,
                cspNote: csp.detail,
                cspState: csp.state,
                hostingState: hosting.state,
                productionGuardStatus: 'not_run'
            }, extra || {});
            return this.capture(TYPES.DEPLOYMENT_HEALTH_CHECKED, 'deployment health probe @ ' + info.origin, info);
        },
