...
(function () {
    ...
    SYSOS.CONFIG = Object.freeze({
        VERSION: '4.5.4',
        CODENAME: 'RLS_TECHNICAL_VERIFICATION_STATUS_PATH',
        PREVIOUS_VERSION: '4.5.2',

        // v4.0 Pilot backend config (placeholders only — NO secrets in the repo).
        // Real values are supplied at runtime via a git-ignored config.local.js
        // setting window.__SYSOS_RUNTIME__.BACKEND. LOCAL is the default; remote
        // is opt-in and never auto-overwrites local data.
        BACKEND: Object.freeze({
            MODE: 'local',                 // 'local' | 'remote_supabase'
            REMOTE_ENABLED: false,         // opt-in only
            SUPABASE_URL: '',              // placeholder — set at runtime, never commit real
            SUPABASE_ANON_KEY: '',         // placeholder — ANON key only, never service-role
            KV_TABLE: 'sysos_kv_state'
        }),

        // v4.2 Client-side monitoring sink. LOCAL-only — events route into the
        // central audit + recent-activity ledger; no external alerting.
        MONITORING: Object.freeze({
            EVENT_CAP: 50,            // rolling cap on retained monitoring events
            ROUTE_TO_AUDIT: true,     // append each event to the central audit trail
            ROUTE_TO_ACTIVITY: true   // surface each event in the recent-activity feed
        }),

        // v4.3 Live-deployment rehearsal. Hosting/RLS readiness metadata. No
        // secrets — real Supabase values come from a git-ignored config.local.js.
        DEPLOY: Object.freeze({
            HOSTING_TARGET: 'GitHub Pages',
            GITHUB_PAGES_HOST_SUFFIX: '.github.io',  // project site host detection
            BASE_PATH_NOTE: 'project site serves at /<repo>/ — verify asset paths resolve',
            ROLLBACK_DOCUMENTED: true,
            RLS_STATUS_KEY: 'sysos.deploy.rls.v1',   // persisted RLS verification/attestation record
            // v4.5.4: a TECHNICALLY_VERIFIED record older than this requires a rerun
            RLS_VERIFICATION_MAX_AGE_DAYS: 30
        }),
        ...
        STORE: Object.freeze({
            ...
            SEED_VERSION: 5,        // v2.9.7: proposal/contract schema fields added — bump forces reseed
            ACTIVITY_CAP: 50
        }),

        // v2.9.6 COMMERCIAL PIPELINE ----------------------------------------

        COMMERCIAL: Object.freeze({
            CLIENT_LIFECYCLE: Object.freeze(['LEAD', 'QUALIFIED', 'PROPOSAL', 'NEGOTIATION', 'WON', 'ACTIVE', 'COMPLETED', 'ARCHIVED']),
            // v2.9.7 proposal lifecycle: Draft -> Review -> Sent -> Accepted/Rejected/Expired
            PROPOSAL_STATUSES: Object.freeze(['DRAFT', 'REVIEW', 'SENT', 'ACCEPTED', 'REJECTED', 'EXPIRED']),
            CONTRACT_STATUSES: Object.freeze(['NOT_SENT', 'SENT', 'SIGNED', 'ACTIVE', 'EXPIRED', 'TERMINATED']),
            // Probability weights for Expected Revenue (proposal status -> close likelihood).
            // A per-proposal `probability` field, when set, overrides this default.
            PROPOSAL_WEIGHTS: Object.freeze({
                DRAFT: 0.1, REVIEW: 0.25, SENT: 0.5, ACCEPTED: 1, REJECTED: 0, EXPIRED: 0
            }),
            // Automation warning thresholds (days remaining) — descending.
            EXPIRY_THRESHOLDS: Object.freeze([30, 14, 7, 3, 1]),
            CONTRACT_ALIVE: Object.freeze(['SIGNED', 'ACTIVE']),
            // Health scoring: 6 factors x 20 wait -> normalized to 100; risk thresholds.
            HEALTH: Object.freeze({
                ACTIVITY_STALE_DAYS: 30,
                GREEN_MIN: 70,
                YELLOW_MIN: 40,
                FACTOR_POINTS: 20
            })
