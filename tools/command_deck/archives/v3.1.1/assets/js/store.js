...
(function () {
    ...
    function persistAll() {
        ...
        if (SYSOS.stores._suspendPersist) return;   // v2.9.9: Demo Mode isolation — no live writes
        try {
            const domains = {};
            Object.keys(SYSOS.stores.all).forEach(function (d) {
                domains[d] = SYSOS.stores.all[d].toJSON();
            });
            // v3.0: write through the storage adapter (localStorage default, SQLite-swappable).
            SYSOS.storage.set(SYSOS.CONFIG.STORE.STORAGE_KEY, JSON.stringify({
                v: 1,
    ...
    SYSOS.activity = {
        ...
        log(type, message) {
            ...
            if (!(SYSOS.stores && SYSOS.stores._suspendPersist)) {   // v2.9.9: no demo writes
                try {
                    SYSOS.storage.set(SYSOS.CONFIG.STORE.ACTIVITY_KEY,
