...
(function () {
    ...
    const scheduler = {
        ...
        schedule(complianceId, opts) {
            opts = opts || {};
            if (SYSOS.auth) SYSOS.auth.enforce('compliance.manage', { domain: 'compliance' });   // v3.3 RBAC
        ...
        persist() {
            ...
            try {
                SYSOS.storage.set(cfg().STORAGE_KEY, JSON.stringify({
        ...
        restore() {
            try {
                const raw = SYSOS.storage.get(cfg().STORAGE_KEY);
                if (!raw) return false;
