...
(function () {
    ...
    const monitoring = {
        ...
        deploymentHealthCheck: function (extra) {
            ...
            const bcfg = Object.assign({}, base, rt);
            let cspNote = 'CSP local-only (no Supabase configured)';
            try {
                if (bcfg.SUPABASE_URL) {
                    const meta = document.querySelector('meta[http-equiv="Content-Security-Policy"]');
                    const csp = meta ? (meta.getAttribute('content') || '') : '';
                    const origin = new URL(bcfg.SUPABASE_URL).origin;
                    cspNote = csp.indexOf(origin) !== -1 ? ('CSP allows ' + origin) : ('CSP MISSING ' + origin);
                }
            } catch (e) { cspNote = 'CSP note unavailable'; }
