...
(function () {
    ...
    const client = {
        ...
        metrics(clientId) {
            ...
            return {
                ...
                documents: documents,
                linkedEntities: SYSOS.relations.backlinks(clientId).length
            };
        },
    ...
    function stationHTML() {
        ...
        '<div class="bg-black/40 border border-gold-500/10 rounded-xl p-6 backdrop-blur-md">' +
            ...
            '</div>' +
            '<div id="client-dash" class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6 text-center"></div>' +
    ...
    function refreshAll() { renderDash(); renderList(); renderWorkspace(); }
    ...
    SYSOS.clientInit = function () {
        ...
        document.addEventListener('sysos:data', function (e) {
            if (!e.detail || e.detail.domain === 'clients' || e.detail.domain === 'projects') refreshAll();
        });
        refreshAll();
    };
})();
