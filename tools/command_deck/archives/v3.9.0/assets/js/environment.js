...
(function () {
    ...
    async function runProductionGuard() {
        ...
        const checks = [
            { name: 'Authentication', status: 'BLOCK', detail: 'no backend authentication — ADMINISTRATOR is the no-password default' },
            { name: 'Server persistence', status: 'BLOCK', detail: 'localStorage only — no durable server store' },
            { name: 'Hosting / HTTPS', status: secure ? 'WARN' : 'BLOCK', detail: secure ? 'secure context OK, but no hosting configured' : 'not a secure context — vault hash would degrade' },
            { name: 'Monitoring', status: 'BLOCK', detail: 'no runtime error capture or alerting' },
