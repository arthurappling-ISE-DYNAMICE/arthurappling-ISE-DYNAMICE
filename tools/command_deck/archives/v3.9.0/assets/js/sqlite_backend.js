...
(function () {
    ...
    SqliteBackend.prototype.export = function () {
        if (SYSOS.audit) SYSOS.audit.record({ action: 'export', domain: 'system', target: 'sqlite(kv)',
            source: 'sqlite', notes: this.table.length + ' rows exported' });
    ...
    SqliteBackend.prototype.import = function (dump) {
        if (SYSOS.auth && !SYSOS.auth.can('storage.migrate')) {
            if (SYSOS.audit) SYSOS.audit.record({ action: 'permission_denied', domain: 'system', target: 'import', source: 'rbac', reason: 'import requires storage.migrate' });
            throw new Error('permission denied: import');
        }
        ...
        this._reindex();
        this._flush();
        if (SYSOS.audit) SYSOS.audit.record({ action: 'import', domain: 'system', target: 'sqlite(kv)',
            source: 'sqlite', notes: this.table.length + ' rows imported' });
    ...
    SYSOS.sqlite = {
        ...
        migrate() {
            ...
            // v3.2: RBAC + audit on migration.
            if (SYSOS.auth && !SYSOS.auth.can('storage.migrate')) {
                if (SYSOS.audit) SYSOS.audit.record({ action: 'permission_denied', domain: 'system', target: 'storage.migrate',
                    source: 'rbac', reason: 'role ' + SYSOS.auth.current.role + ' lacks storage.migrate' });
                throw new Error('permission denied: storage.migrate');
            }
            const before = snapshot();
            if (SYSOS.audit) SYSOS.audit.record({ action: 'migration_start', domain: 'system', target: 'sqlite(kv)',
                before: before, source: 'migration-engine', notes: 'localStorage -> sqlite' });
            ...
            if (SYSOS.audit) SYSOS.audit.record({ action: 'migration_complete', domain: 'system', target: 'sqlite(kv)',
                before: before, after: after, source: 'migration-engine',
                reason: parity ? 'parity verified' : 'PARITY MISMATCH', notes: parity ? 'parity OK' : 'PARITY MISMATCH' });
