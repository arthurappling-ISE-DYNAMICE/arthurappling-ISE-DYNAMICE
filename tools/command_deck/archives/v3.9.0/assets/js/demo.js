...
(function () {
    ...
    const demo = {
        ...
        enter() {
            ...
            // v3.9: production guard — demo mode is blocked in PRODUCTION profile.
            if (SYSOS.environment && SYSOS.environment.blocks('demo')) {
                if (SYSOS.audit) SYSOS.audit.record({ action: 'environment.demo_blocked', domain: 'environment',
                    target: 'demo', source: 'demo', reason: 'demo blocked in ' + SYSOS.environment.getCurrentProfile() + ' profile' });
                if (SYSOS.utils) SYSOS.utils.notify('DEMO BLOCKED', ['Demo mode is disabled in ' + SYSOS.environment.getCurrentProfile() + ' profile.']);
                return demo.status();   // stays LIVE — no sandbox entered
            }
