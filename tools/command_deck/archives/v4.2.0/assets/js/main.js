...
(function () {
    ...
    async function boot() {
        ...
        // v3.1: re-activate the SQLite backend before any restore if it was active.
        if (SYSOS.sqlite && SYSOS.sqlite.activateFromFlag()) {
            console.info('[SYS_OS] storage backend: ' + SYSOS.storage.backendName + ' (restored from flag)');
        }
        bindMetrics();
        bindActions();
        SYSOS.canvas.init();
        ...
        if (SYSOS.audit) SYSOS.audit.init();                            // v3.1 audit history (wrap post-seed)
        if (SYSOS.monitoring) SYSOS.monitoring.init();                  // v4.2 runtime monitoring sink (listeners pre-installed; LOCAL-only)
        // v3.5 H2: record the vault hash mode as a startup audit event (after
        // audit.init so restore() does not overwrite it).
        if (SYSOS.audit && SYSOS.vault && typeof SYSOS.vault.hashMode === 'function') {
            const hm = SYSOS.vault.hashMode();
            SYSOS.audit.record({ action: 'vault.hash_mode', domain: 'documents', target: 'vault',
                source: 'vault', notes: 'hash mode ' + hm.algo + (hm.strong ? ' (cryptographic)' : ' (NON-CRYPTOGRAPHIC FALLBACK)'),
                metadata: { algo: hm.algo, strong: hm.strong, secureContext: hm.secureContext } });
        }
        // v3.5 H4: flush any restore-quarantine audit events queued during boot
        // (vault.init runs before audit.init).
        if (SYSOS.vault && typeof SYSOS.vault.flushRestoreAudit === 'function') SYSOS.vault.flushRestoreAudit();
        if (SYSOS.authUI) SYSOS.authUI.init();                          // v3.1 access control station 13
        if (SYSOS.backup) SYSOS.backup.init();                          // v3.8 backup/restore station 14 (guarded; not boot-gated)
        if (SYSOS.environment) SYSOS.environment.init();                // v3.9 environment profiles + production guard station 15 (guarded; not boot-gated)
        if (SYSOS.authRemote) SYSOS.authRemote.init();                  // v4.0 remote auth foundation (no-op if Supabase not configured)
        if (SYSOS.remoteBackend) SYSOS.remoteBackend.init();            // v4.0 pilot backend station 16 (guarded; not boot-gated; LOCAL preserved)
