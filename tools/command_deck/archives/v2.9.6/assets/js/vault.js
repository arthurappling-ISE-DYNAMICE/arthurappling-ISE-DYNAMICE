...
(function () {
    ...
    /** Document relationship links (projects/workflows/agents/clients own the doc). */
    function normalizeDocLinks(links) {
        return {
            projects: (links && Array.isArray(links.projects)) ? links.projects.slice() : [],
            workflows: (links && Array.isArray(links.workflows)) ? links.workflows.slice() : [],
            agents: (links && Array.isArray(links.agents)) ? links.agents.slice() : [],
            clients: (links && Array.isArray(links.clients)) ? links.clients.slice() : [],  // v2.9.5
            documents: []
        };
    }
