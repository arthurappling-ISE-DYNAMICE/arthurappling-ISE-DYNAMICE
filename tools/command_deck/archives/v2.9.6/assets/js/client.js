...
(function () {
    ...
    const client = {
        ...
        metrics(clientId) {
            ...
            // D-1 FIX (v2.9.6): linkedEntities now measures BOTH directions —
            // forward links declared on the client record + reverse backlinks
            // from any entity pointing at the client. Previously reverse-only.
            const forwardLinks = ['projects', 'workflows', 'agents', 'documents', 'clients']
                .reduce(function (n, d) { return n + (resolved.links[d] ? resolved.links[d].length : 0); }, 0);
            const reverseLinks = SYSOS.relations.backlinks(clientId).length;
            return {
                clientId: clientId,
                name: c.name,
                segment: c.segment,
                ...
                linkedEntities: forwardLinks + reverseLinks
            };
        },
