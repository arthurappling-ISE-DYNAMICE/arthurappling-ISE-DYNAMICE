...
(function () {
    ...
    function defineStores() {
        ...
        // v2.9.6: stage + stageHistory added for the lifecycle engine.
        SYSOS.stores.define('clients', {
            label: 'Client Registry', idPrefix: 'CLT',
            required: ['name', 'segment', 'status'],
            defaults: { contact: '', email: '', value: 0, notes: '', tier: 'STANDARD', stage: 'LEAD', stageHistory: [] }
        });
        // v2.9.6 Commercial Pipeline domains. v2.9.7: schema expanded.
        SYSOS.stores.define('proposals', {
            label: 'Proposal Registry', idPrefix: 'PRO',
            required: ['clientId', 'title', 'amount', 'status'],
            defaults: { description: '', probability: null, owner: 'A. F. Appling Sr.',
                notes: '', createdDate: '', proposalDate: '', expirationDate: '' }
        });
        SYSOS.stores.define('contracts', {
            label: 'Contract Registry', idPrefix: 'CON',
            required: ['clientId', 'title', 'status', 'value'],
            defaults: { proposalId: null, owner: 'A. F. Appling Sr.',
    ...
    function seed() {
        ...
        s.clients.create({ id: 'house_account', name: 'AA Capital INC dba Prime Pathwy', segment: 'HOUSE', tier: 'INTERNAL',
            status: 'ACTIVE', stage: 'ACTIVE', stageHistory: [{ from: null, to: 'ACTIVE', ts: '2026-01-01T00:00:00.000Z' }],
            contact: 'A. F. Appling Sr.', email: 'arthurappling@gmail.com', value: 0,
        ...
        s.clients.create({ id: 'sample_consulting_a', name: 'SAMPLE — Consulting Client A', segment: 'CONSULTING', tier: 'ELITE',
            status: 'PROSPECT', stage: 'WON', stageHistory: [
                { from: null, to: 'LEAD', ts: '2026-05-01T00:00:00.000Z' },
                { from: 'LEAD', to: 'QUALIFIED', ts: '2026-05-10T00:00:00.000Z' },
                { from: 'QUALIFIED', to: 'PROPOSAL', ts: '2026-05-18T00:00:00.000Z' },
                { from: 'PROPOSAL', to: 'WON', ts: '2026-06-01T00:00:00.000Z' }],
            contact: 'TBD', email: '', value: 5000,
        ...
        s.clients.create({ id: 'sample_ecom_b', name: 'SAMPLE — E-Commerce Client B', segment: 'E-COMMERCE', tier: 'STANDARD',
            status: 'PROSPECT', stage: 'PROPOSAL', stageHistory: [
                { from: null, to: 'LEAD', ts: '2026-05-20T00:00:00.000Z' },
                { from: 'LEAD', to: 'PROPOSAL', ts: '2026-06-05T00:00:00.000Z' }],
            contact: 'TBD', email: '', value: 2500,
        ...
        // PROPOSALS (v2.9.6/.7) — links.clients + links.proposals for symmetric resolution.
        // Dates relative to "now" so automation thresholds are exercised on any boot.
        var soon = function (days) { return new Date(Date.now() + days * 86400000).toISOString().slice(0, 10); };
        s.proposals.create({ id: 'PRO-2026-A', clientId: 'sample_consulting_a',
            title: 'AI Consulting Engagement — Elite 10 Sovereign Deck', description: 'NEPQ-qualified consulting engagement.',
            amount: 5000, status: 'ACCEPTED', probability: null, owner: 'A. F. Appling Sr.',
            createdDate: '2026-05-18', proposalDate: '2026-05-20', expirationDate: soon(45),
            notes: 'Accepted; advanced to contract CON-2026-A.',
            links: { clients: ['sample_consulting_a'], projects: ['ai_consulting_framework'] } }, quiet);
        s.proposals.create({ id: 'PRO-2026-B', clientId: 'sample_ecom_b',
            title: 'Shopify Platform Build — Storefront + Payment Rails', description: 'Storefront + payment rails build-out.',
            amount: 2500, status: 'SENT', probability: null, owner: 'A. F. Appling Sr.',
            createdDate: soon(-20), proposalDate: soon(-18), expirationDate: soon(10),  // expires in ~10d -> 14d warning
            notes: 'Sent; awaiting client decision.',
            links: { clients: ['sample_ecom_b'], projects: ['shopify_platform'] } }, quiet);

        // CONTRACTS (v2.9.6/.7) — clientId + proposalId, renewalDate for monitoring.
        s.contracts.create({ id: 'CON-2026-A', clientId: 'sample_consulting_a', proposalId: 'PRO-2026-A',
            title: 'Consulting Master Services Agreement', status: 'SIGNED', value: 5000, owner: 'A. F. Appling Sr.',
            effectiveDate: soon(-12), expirationDate: soon(20), renewalDate: soon(6),   // renews in ~6d -> 7d alert
            links: { clients: ['sample_consulting_a'], proposals: ['PRO-2026-A'] } }, quiet);
    ...
    const REF_FIELD = {
        ...
        proposals: function (e) { return '$' + (e.amount || 0).toLocaleString() + ' · ' + e.clientId; },  // v2.9.6
        contracts: function (e) { return '$' + (e.value || 0).toLocaleString() + ' · ' + e.clientId; }     // v2.9.6
    };

    let selectedDomain = 'projects';
    let panelRef = null;

    // v3.5 Phase 4: Registry Grid pagination — mirrors the proven Client Center
    // pattern (client.js CLIENT_PAGE_SIZE). Filter/search apply to the full
    // dataset at the data layer (store.list), then only one page of rows is
    // rendered to the DOM. registryPage is 0-indexed and resets to 0 on any
    // domain switch or filter change.
    ...
    function renderTable() {
        ...
        // v3.5 Phase 4: filter the FULL dataset at the data layer first, THEN
        // paginate — only one page of rows is ever rendered to the DOM, so no
        // off-page records exist as hidden DOM nodes.
        const total = store.list({ includeArchived: true }).length;   // domain total (unfiltered)
        const all = store.list(currentFilter());                       // full filtered/searched set
        const filtered = all.length;
        const pages = Math.max(1, Math.ceil(filtered / REGISTRY_PAGE_SIZE));
        if (registryPage >= pages) registryPage = pages - 1;
        if (registryPage < 0) registryPage = 0;
        const start = registryPage * REGISTRY_PAGE_SIZE;
        const pageRows = all.slice(start, start + REGISTRY_PAGE_SIZE);

        if (!filtered) {
            const tr = el('tr');
            const td = el('td', 'p-4 text-[11px] font-mono text-slate-600');
            td.colSpan = 5;
            td.textContent = 'No records match the current filter.';
            tr.appendChild(td);
            body.appendChild(tr);
            return;
        }
        ...
        // v3.5 Phase 4: pagination + count footer as a table-valid row.
        // Count display (req 9): filtered-window · filtered total · domain total · page/pages.
        const footTr = el('tr', 'bg-slate-950/40');
        const footTd = el('td', 'p-3');
        footTd.colSpan = 5;
        const bar = el('div', 'flex items-center justify-between gap-2');
        const prev = el('button', 'text-[10px] font-mono border border-white/15 text-slate-400 px-3 py-1 rounded tracking-widest hover:text-slate-200 disabled:opacity-30', 'PREV');
        prev.type = 'button'; prev.dataset.regPage = 'prev'; prev.disabled = registryPage === 0;
        const info = el('div', 'text-[10px] font-mono text-slate-500 tracking-wider',
            (start + 1) + '–' + Math.min(start + REGISTRY_PAGE_SIZE, filtered) +
            ' of ' + filtered + ' filtered · ' + total + ' total · pg ' + (registryPage + 1) + '/' + pages);
        const next = el('button', 'text-[10px] font-mono border border-white/15 text-slate-400 px-3 py-1 rounded tracking-widest hover:text-slate-200 disabled:opacity-30', 'NEXT');
        next.type = 'button'; next.dataset.regPage = 'next'; next.disabled = registryPage >= pages - 1;
        bar.appendChild(prev); bar.appendChild(info); bar.appendChild(next);
        footTd.appendChild(bar);
        footTr.appendChild(footTd);
        body.appendChild(footTr);
    }

    // v3.5 Phase 4: a new search/filter result starts at page 1.
    function renderTableFirstPage() { registryPage = 0; renderTable(); }
    ...
    SYSOS.registriesInit = function () {
        panelRef = SYSOS.router.register({
            ...
            // v3.3: lazy — full setup runs on first activation, not at boot.
            render: function (panel) {
                panel.innerHTML = stationHTML();   // static trusted template
                panel.addEventListener('click', function (e) {
                    // v3.5 Phase 4: pagination controls
                    const pg = e.target.closest('[data-reg-page]');
                    if (pg) {
                        registryPage += (pg.dataset.regPage === 'next' ? 1 : -1);
                        renderTable();
                        return;
                    }
                    ...
                    if (card) {
                        ...
                        registryPage = 0;   // v3.5 Phase 4: switching domains resets to page 1
                        document.getElementById('registry-detail').classList.add('hidden');
                        closeForm();
                        refresh();
                        return;
                ...
                panel.querySelector('#registry-form').addEventListener('submit', function (e) {
                    e.preventDefault();
                    const form = e.target;
                    const data = {
                ...
                panel.querySelector('#registry-search').addEventListener('input', SYSOS.utils.debounce(renderTableFirstPage, 150));
                panel.querySelector('#registry-status-filter').addEventListener('change', renderTableFirstPage);
                refresh();
            }
        });

        // Refresh only re-renders if the station has been mounted (elements exist);
        // otherwise the render functions no-op (guarded by element checks).
        document.addEventListener('sysos:data', function () { if (panelRef && panelRef.querySelector('#registry-cards')) refresh(); });
    };
