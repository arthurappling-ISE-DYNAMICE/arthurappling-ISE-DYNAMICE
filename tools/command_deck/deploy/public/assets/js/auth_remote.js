...
(function () {
    ...
    // v4.4.3: backend-config merge now lives in one place — runtime_config.js.
    function cfg() { return SYSOS.runtimeConfig.getBackendConfig(); }

    let _client = null;
    let _clientOrigin = null;   // origin the cached client was built for
    /**
     * v4.5.2: client creation is gated on the VALIDATED config (not raw
     * non-empty strings) — a placeholder/malformed URL or a blocked key can no
     * longer instantiate a client, so signIn() can never fire at an invalid
     * host. The cache is keyed by origin and reset whenever the config becomes
     * invalid or points at a different project.
     */
    function client() {
        const supa = SYSOS.runtimeConfig.isSupabaseConfigured();
        if (!supa.ok) { _client = null; _clientOrigin = null; return null; }
        if (_client && _clientOrigin === supa.origin) return _client;
        _client = null; _clientOrigin = null;
        const c = cfg();
        try { _client = window.supabase.createClient(c.SUPABASE_URL, c.SUPABASE_ANON_KEY); _clientOrigin = supa.origin; }
        catch (e) { console.warn('[SYS_OS:auth_remote] client init failed', e); _client = null; _clientOrigin = null; }
        return _client;
    }
    ...
    const authRemote = {
        /**
         * SDK + config + enabled + VALID SHAPE all present (v4.4.3: routed through
         * runtime_config's format validation — a malformed URL/key no longer
         * reports "configured").
         */
        isConfigured: function () { return !!SYSOS.runtimeConfig.isSupabaseConfigured().ok; },
        ...
        /** Honest status object for the UI / guard (v4.4.3: format-validated, not just non-empty). */
        getAuthStatus: async function () {
            ...
            const supa = SYSOS.runtimeConfig.isSupabaseConfigured();
            if (!supa.ok) return { status: supa.status, configured: false, authenticated: false,
                detail: supa.reason || (supa.status === 'LOCAL' ? 'remote not enabled (LOCAL mode)' : 'Supabase URL/anon key not configured (runtime config required)') };
